...
import { experticias, personasCasos, personaTelefonos, insertExperticiasSchema } from '../shared/schema';
import { db } from './db';
...
export function registerDocumentRoutes(app: Express, authenticateToken: any, storage: any) {
  ...
  // POST /api/experticias/completa - Crea experticia + personas + teléfonos en una sola transacción atómica
  app.post("/api/experticias/completa", authenticateToken, async (req: any, res) => {
    try {
      if (req.user.rol !== 'admin') {
        return res.status(403).json({ message: "No tienes permisos para crear experticias" });
      }

      const { personas = [], telefonos = [], ...experticiaBody } = req.body;

      // Verificar duplicado ANTES de abrir la transacción (lectura sin bloqueo)
      const numeroDictamen = (experticiaBody.numeroDictamen || '').trim().toUpperCase();
      const existentes = await storage.getExperticias({ search: numeroDictamen, limit: 1 });
      if (existentes.total > 0 && existentes.experticias.length > 0) {
        return res.status(400).json({ message: 'Ya existe una experticia con este número de dictamen' });
      }

      // Validar campos de la experticia
      const validation = insertExperticiasSchema.safeParse(experticiaBody);
      if (!validation.success) {
        return res.status(400).json({ message: "Datos inválidos", errors: validation.error.errors });
      }

      // ── TRANSACCIÓN ATÓMICA ────────────────────────────────────────────────
      // Si cualquier paso falla, todo se revierte (rollback automático).
      const experticia = await db.transaction(async (tx) => {
        // 1. Crear la experticia
        const [nuevaExperticia] = await tx
          .insert(experticias)
          .values({ ...validation.data, usuarioId: req.user.id })
          .returning();

        // 2. Guardar / actualizar personas
        for (const persona of personas) {
          ...
          const [existente] = await tx
            .select()
            .from(personasCasos)
            .where(eq(personasCasos.cedula, cedula));

          const bioData = {
            cedula,
            nombre: persona.nombre || null,
            apellido: persona.apellido || null,
            pseudonimo: persona.pseudonimo || null,
            fechaDeNacimiento: persona.fechaDeNacimiento || null,
            correo: persona.correo || null,
            direccion: persona.direccion || null,
            statusLinea: persona.statusLinea || null,
            fechaActivacion: persona.fechaActivacion || null,
            otrosTlf: persona.otrosTlf || null,
            rol: persona.rol || null,
            profesion: persona.profesion || null,
            delito: persona.delito || null,
            fiscalia: persona.fiscalia || null,
            telefono: persona.telefono || null,
            expediente: persona.expediente || null,
          };
          ...
          if (existente) {
            await tx
              ...
              .where(eq(personasCasos.nro, existente.nro));
          } else {
            await tx.insert(personasCasos).values(bioData);
          }
        }

        // 3. Actualizar datos de teléfonos (statusLinea / fechaActivacion)
        for (const tel of telefonos) {
          const numero = (tel.numero || '').trim();
          if (!numero) continue;
          ...
          if (existente) {
            await tx
              ...
              .set({ statusLinea: tel.statusLinea || null, fechaActivacion: tel.fechaActivacion || null })
              .where(eq(personaTelefonos.id, existente.id));
          }
        }

        return nuevaExperticia;
      });
      // ── FIN DE TRANSACCIÓN ─────────────────────────────────────────────────

      // Normalizar registros de comunicación (operación no crítica - fuera de transacción)
      const listaAnalisis: any[] = Array.isArray(req.body.listaAnalisis) ? req.body.listaAnalisis : [];
      if (listaAnalisis.length > 0) {
        try {
          ...
          for (const item of listaAnalisis) {
            ...
            if (!telAnalizado) {
              telAnalizado = await storage.createPersonaTelefono({ numero, tipo: "móvil", activo: true, personaId: null });
            }
            ...
            const filasMapeadas = datosCrudos
              .map((row: any) => {
                ...
                mapped.abonadoAId = telAnalizado!.id;
          ...
          console.error("[EXPERTICIA COMPLETA] Error normalizando registros:", normError?.message);
        }
      }

      res.status(201).json(experticia);
    } catch (error: any) {
      console.error("Error en /api/experticias/completa:", error);
      if (error.code === '23505') {
